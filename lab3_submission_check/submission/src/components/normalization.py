...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        rms = torch.sqrt(torch.mean(x**2, dim=-1, keepdim=True) + self.eps)  # STUDENT TODO
...
class PreNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        return x + self.fn(self.norm(x), **kwargs)  # STUDENT TODO
...
class PostNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        return self.norm(x + self.fn(x, **kwargs))  # STUDENT TODO
