...
def create_causal_mask(seq_len: int, device: torch.device) -> torch.Tensor:
    ...
    mask = mask.view(1, 1, seq_len, seq_len)# mask.unsqueeze(0).unsqueeze(0)  # STUDENT TODO
...
def create_padding_mask(seq: torch.Tensor, pad_idx: int = 0) -> torch.Tensor:
    ...
    mask = (seq != pad_idx).int()  # STUDENT TODO

    # TODO: Reshape to (batch_size, 1, 1, seq_len) for broadcasting
    mask = mask.view(seq.size(0), 1, 1, seq.size(1))  # STUDENT TODO
